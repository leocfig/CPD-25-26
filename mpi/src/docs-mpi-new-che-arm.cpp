...
void update_step(const double* __restrict__ docs, double* __restrict__ centroids,
  ...
  // Offsets inside mpi_recv_buf
  const size_t off_counts  = (size_t)task_nr_cents * S;
  const size_t off_changed = off_counts + task_nr_cents;
  const size_t off_dirty   = off_changed + 1;
  const size_t buf_size    = off_dirty + task_nr_cents;
  ...
  for (uint k = 0; k < task_nr_cents; k++) {
    ...
    mpi_recv_buf[off_counts + k] = sum_count;
    ...
    // Pack dirty flag into buffer so it gets reduced along with sums/counts
    mpi_recv_buf[off_dirty + k] = 1.0;
  }

  #pragma omp single
  {
    // Append changed_count at the end of the buffer so it gets reduced together with sums and counts
    mpi_recv_buf[off_changed] = (double)changed_count;

    double t_comm = -MPI_Wtime();

    MPI_Allreduce(MPI_IN_PLACE, mpi_recv_buf, (int)buf_size,
                  MPI_DOUBLE, MPI_SUM, g.col_comm);

    double global_changed = mpi_recv_buf[off_changed];
  ...
  // Use the globally-reduced dirty flags (off_dirty) instead of the local dirty[],
  // so we also update clusters that were dirty on other col_comm processes.
  #pragma omp for
  for (uint k = 0; k < task_nr_cents; k++) {
    if (mpi_recv_buf[off_dirty + k] == 0.0) continue;  // nobody touched this cluster globally

    double count = mpi_recv_buf[off_counts + k];
...
void reassign_step(const double* __restrict__ docs, const double* __restrict__ centroids,
  ...
  // Compute local best cluster per document (non-AVX)
  #pragma omp for
  for (uint block_idx = 0; block_idx < D_padded; block_idx += BLOCK_SIZE) {
    ...
    for (uint lane = 0; lane < BLOCK_SIZE; lane++) {
      ...
      best_cluster[lane] = task_first_cent;
    }

    // Unrolled centroid loop (4 at a time) — only over local centroids
    uint c = 0;
    for (; c + 4 <= C_padded_local; c += 4) {
      double dist0[BLOCK_SIZE] = {};
      double dist1[BLOCK_SIZE] = {};
      double dist2[BLOCK_SIZE] = {};
      double dist3[BLOCK_SIZE] = {};
      ...
      for (uint s = 0; s < S; ++s) {
        ...
        for (uint lane = 0; lane < BLOCK_SIZE; lane++) {
          ...
          double d0 = v - cent0, d1 = v - cent1;
          double d2 = v - cent2, d3 = v - cent3;
          dist0[lane] += d0 * d0;
          dist1[lane] += d1 * d1;
          dist2[lane] += d2 * d2;
          dist3[lane] += d3 * d3;
      ...
      // Argmin update for 4 centroids
      #pragma GCC ivdep
      for (uint lane = 0; lane < BLOCK_SIZE; lane++) {
        if (dist0[lane] < min_dist[lane]) { min_dist[lane] = dist0[lane]; best_cluster[lane] = task_first_cent + c + 0; }
        if (dist1[lane] < min_dist[lane]) { min_dist[lane] = dist1[lane]; best_cluster[lane] = task_first_cent + c + 1; }
        if (dist2[lane] < min_dist[lane]) { min_dist[lane] = dist2[lane]; best_cluster[lane] = task_first_cent + c + 2; }
        if (dist3[lane] < min_dist[lane]) { min_dist[lane] = dist3[lane]; best_cluster[lane] = task_first_cent + c + 3; }
      }
    }

    // Tail loop for remaining centroids
    for (; c < C_padded_local; ++c) {
      ...
      for (uint lane = 0; lane < BLOCK_SIZE; lane++) {
        if (dist[lane] < min_dist[lane]) {
          ...
          best_cluster[lane] = task_first_cent + c;
        }
      }
    }

    // Pack into local_pairs for MPI_Allreduce MPI_MINLOC
    for (uint i = 0; i < BLOCK_SIZE; i++)
      local_pairs[block_idx + i] = { min_dist[i], (int)best_cluster[i] };
  }

  // Global reduction: each row picks the globally nearest centroid per document
  #pragma omp single
  {
    MPI_Allreduce(MPI_IN_PLACE, local_pairs, (int)D_padded, MPI_DOUBLE_INT, MPI_MINLOC, g.row_comm);
  }

  // Update assignments and mark dirty clusters
  #pragma omp for reduction(|:changed_count)
  for (uint block_idx = 0; block_idx < D_padded; block_idx += BLOCK_SIZE) {
    int valid_docs = (int)std::min((uint)BLOCK_SIZE, task_nr_docs > block_idx ? task_nr_docs - block_idx : 0u);

    for (uint i = 0; i < (uint)valid_docs; i++) {
      uint new_k = (uint)local_pairs[block_idx + i].idx;
      uint old_k = assigns[block_idx + i];

      if (new_k != old_k) {
        changed_count |= 1;  // match original: OR instead of increment

        // Mark old cluster dirty (if owned by this process)
        if (old_k >= task_first_cent && old_k < task_first_cent + task_nr_cents)
          dirty[old_k - task_first_cent] = true;

        // Mark new cluster dirty (if owned by this process)
        if (new_k >= task_first_cent && new_k < task_first_cent + task_nr_cents)
          dirty[new_k - task_first_cent] = true;

        assigns[block_idx + i] = new_k;
      }
    }
  }
}
...
int main(int argc, char** argv) {
  ...
  std::fill_n(mpi_recv_buf.get(), recv_buf_size, 0.0);
  ...
  { 
    ...
    // First update: all clusters dirty, computes initial centroids from round-robin assignments
    update_step(docs.get(), centroids.get(), assignments.get(),
                ...
                mpi_recv_buf.get(), dirty.get(),
                changed_count, changed, total_comm_time, g);

    while (changed) {
      #pragma omp single
      changed_count = 0;

      // reassign_step resets dirty[] and re-marks only clusters that gained/lost documents
      reassign_step(docs.get(), centroids.get(), assignments.get(),
                    C_padded_local, task_nr_docs, task_nr_cents, D_padded, S,
                    task_first_cent, local_pairs, dirty.get(), changed_count, g);
