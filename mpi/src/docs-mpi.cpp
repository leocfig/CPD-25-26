...
void parse_input(std::ifstream& in_stream, AlignedPtr<uint>& assigns, AlignedPtr<double>& docs,
                 uint& C, uint& D, uint& S, int id, int num_procs, uint& task_nr_docs, uint& task_first_doc) {
  ...
  // Skip documents not meant for this task
  uint doc_id;
  double skip;
  for (uint d = 0; d < task_first_doc; d++) {
    in_stream >> doc_id;
    for (uint s = 0; s < S; s++) in_stream >> skip;
  }
  ...
  // Only reads the documents meant for this task
...
AlignedPtr<uint> gather_results(const uint* local_assigns, uint task_nr_docs, uint D, int id, int num_procs) {
  if (!id) {
    AlignedPtr<int> recvcounts = make_aligned<int>(num_procs);
    AlignedPtr<int> displs     = make_aligned<int>(num_procs);

    displs[0] = 0;
    for (int i = 0; i < num_procs; i++) {
      recvcounts[i] = (int)DOCS_SIZE(i, num_procs, D);
...
void update_step(const double* __restrict__ docs, double* __restrict__ centroids,
  ...
  #pragma omp for
  for (uint k = 0; k < C; k++) {
    double count = mpi_recv_buf[C * S + k];
