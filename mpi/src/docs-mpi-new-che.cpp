...
void reassign_step(const double* __restrict__ docs, const double* __restrict__ centroids,
  ...
  for (uint block_idx = 0; block_idx < D_padded; block_idx += BLOCK_SIZE) {
    ...
    // C_padded_local is always a multiple of 4, so no remainder loop needed.
    for (uint c = 0; c < C_padded_local; c += 4) {
      ...
      __m256d idx0 = _mm256_set1_pd(static_cast<double>(task_first_cent + c + 0));
      __m256d idx1 = _mm256_set1_pd(static_cast<double>(task_first_cent + c + 1));
      __m256d idx2 = _mm256_set1_pd(static_cast<double>(task_first_cent + c + 2));
      __m256d idx3 = _mm256_set1_pd(static_cast<double>(task_first_cent + c + 3));
    ...
    alignas(32) double da[4], db[4], ia[4], ib[4];
    _mm256_store_pd(da, min_dist_A); _mm256_store_pd(ia, best_cluster_A);
    _mm256_store_pd(db, min_dist_B); _mm256_store_pd(ib, best_cluster_B);

    for (uint i = 0; i < 4; i++) {
      local_pairs[block_idx + i    ] = { da[i], (int)ia[i] };
      local_pairs[block_idx + i + 4] = { db[i], (int)ib[i] };
    }
  }
  ...
  for (uint block_idx = 0; block_idx < D_padded; block_idx += BLOCK_SIZE) {
    // Retrieve the 4 + 4 indices as (32-bit integer) and merge into 256-bit register
    alignas(32) int new_ids[BLOCK_SIZE];
    for (uint i = 0; i < BLOCK_SIZE; i++)
      new_ids[i] = local_pairs[block_idx + i].idx;

    __m256i new_idx_256 = _mm256_load_si256((__m256i*)new_ids);
    ...
    int valid_docs = std::min(BLOCK_SIZE, std::max(0, (int)task_nr_docs - (int)block_idx));
    ...
    if (changed_bits) {
      ...
      for (uint i = 0; i < (uint)valid_docs; i++) {
        if (!((changed_bits >> i) & 1)) continue;
        uint old_k = (uint)old_raw[i];
        uint new_k = (uint)new_ids[i];
